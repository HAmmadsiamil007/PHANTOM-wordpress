...
namespace PhantomCore\Settings;

defined('ABSPATH') || exit;

/**
 * Settings Loader — modular section definitions extracted from the 5860-line Settings_Registry.
 *
 * Each section is a public method returning its settings array.
 * The Settings_Registry::define_entries() delegates to this loader.
 *
 * ChatGPT P3: Modularizes the Settings Registry into smaller, maintainable pieces.
 * Each section can later be split into its own file without changing the public API.
 */
class Settings_Loader {

  /**
   * Get all section definitions, keyed by section name.
   *
   * @return array<string, array<string, array>> Section name => { setting_key => entry }
   */
  public function get_all_sections(): array {
    // Core brand & structure
    $sections['branding']         = $this->section_branding();
    $sections['header']           = $this->section_header();
    $sections['topbar']           = $this->section_topbar();
    $sections['navigation']       = $this->section_navigation();
    $sections['announcement_bar'] = $this->section_announcement_bar();

    // Hero & collections
    $sections['hero']              = $this->section_hero();
    $sections['collections']       = $this->section_collections();
    $sections['home_sections']     = $this->section_home_sections();

    // Shop & products
    $sections['shop_page']         = $this->section_shop_page();
    $sections['product_page']      = $this->section_product_page();
    $sections['product_cards']     = $this->section_product_cards();
    $sections['woocommerce']       = $this->section_woocommerce();

    // Blog
    $sections['blog']              = $this->section_blog();

    // Footer
    $sections['footer']            = $this->section_footer();

    // Design
    $sections['typography']        = $this->section_typography();
    $sections['colors']            = $this->section_colors();
    $sections['buttons']           = $this->section_buttons();
    $sections['forms']             = $this->section_forms();
    $sections['spacing']           = $this->section_spacing();
    $sections['layout']            = $this->section_layout();
    $sections['responsive']        = $this->section_responsive();

    // Effects
    $sections['animations']        = $this->section_animations();
    $sections['effects_3d']        = $this->section_effects_3d();

    // Features
    $sections['search']            = $this->section_search();
    $sections['performance']       = $this->section_performance();
    $sections['seo']               = $this->section_seo();
    $sections['accessibility']     = $this->section_accessibility();
    $sections['integrations']      = $this->section_integrations();
    $sections['custom_code']       = $this->section_custom_code();
    $sections['import_export']     = $this->section_import_export();

    // Pages
    $sections['about_page']        = $this->section_about_page();
    $sections['contact_page']      = $this->section_contact_page();
    $sections['faq_page']          = $this->section_faq_page();
    $sections['coming_soon']       = $this->section_coming_soon();
    $sections['error_404']         = $this->section_error_404();
    $sections['login_page']        = $this->section_login_page();
    $sections['register_page']     = $this->section_register_page();
    $sections['portfolio']         = $this->section_portfolio();
    $sections['thank_you']         = $this->section_thank_you();
    $sections['load_more']         = $this->section_load_more();
    $sections['privacy']           = $this->section_privacy();
    $sections['terms']             = $this->section_terms();
    $sections['team']              = $this->section_team();
    $sections['testimonials']      = $this->section_testimonials();

    // Template pack
    $sections['template_pack']     = $this->section_template_pack();

    // Design tokens (no duplicate settings — duplicates removed per ChatGPT P3 fix)
    $sections['design_tokens']     = $this->section_design_tokens();
...
'render_callback' => 'phantom_render_footer_partial',
  ...
  private function section_colors(): array {
    return array(
      ...
      'color_badge_sale_text'         => array(
        'section'      => 'colors',
        'type'     => 'ast-color',
        'default'      => '#ffffff',
        'sanitize'     => 'sanitize_hex_color',
        'label'        => __( 'Sale Badge Text', 'phantom-core' ),
        'css_property' => '--product-badge-sale-text',
        'css_selector' => ':root',
      ),
      ...
      'color_badge_new_text'         => array(
        'section'      => 'colors',
        'type'     => 'ast-color',
        'default'      => '#ffffff',
        'sanitize'     => 'sanitize_hex_color',
        'label'        => __( 'New Badge Text', 'phantom-core' ),
        'css_property' => '--product-badge-new-text',
        'css_selector' => ':root',
      ),
    );
  }
  ...
  private function section_search(): array {
    return array(
      ...
      'search_excerpt_length' => array(
        ...
        'default'  => 20,
        'sanitize' => 'absint',
        'label'    => __( 'Search Excerpt Length', 'phantom-core' ),
      ),
    );
  }
  ...
  /**
   * Design Tokens section — auto-generated from token definitions.
   *
   * @return array
   */
  private function section_design_tokens(): array {
    ...
    foreach ($tokenDefs as $name => $def) {
      ...
      $entry = [
        ...
        'label' => $def['description'] ?? ucwords(str_replace(['_', '.'], ' ', $name)),
        'sanitize' => match ($type) {
...
/**
   ...
   * Used by custom_code and import_export sections.
   *
   * @return \Closure
   */
  public function sanitize_code_passthrough(): \Closure {
    return function ( $v ) {
