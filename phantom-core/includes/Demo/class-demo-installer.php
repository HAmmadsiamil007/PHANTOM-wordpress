...
class Demo_Installer {
    ...
    public function install(string $zip_path): Result {
        if (!file_exists($zip_path)) {
            return Result::fail('ZIP file not found.', ['file_not_found' => $zip_path]);
        }
        ...
        if ($res !== true) {
            return Result::fail('Failed to open ZIP archive.', ['zip_error' => $res]);
        }
        ...
        if (!mkdir($target, 0755, true) && !is_dir($target)) {
            $zip->close();
            return Result::fail('Failed to create target directory.', ['target' => $target]);
        }
    ...
    public function delete(string $slug): Result {
        if ($slug === 'kids') {
            return Result::fail('Cannot delete the default demo.', ['protected_demo' => 'kids']);
        }
    ...
    public function validate_zip(string $zip_path): Result {
        if (!file_exists($zip_path)) {
            return Result::fail('ZIP file not found.');
        }
        ...
        return Result::ok('ZIP file is valid.', [
            'slug' => $slug,
