  ...
  w.PhantomAdapters.PostAdapter = {
    normalize: function(raw) {
      ...
      return {
        ...
        url: p.link || '/?post_id=' + (p.id || ''),
        image: p.image || p.thumbnail || '',
        image_alt: p.title || p.post_title || '',
        excerpt: p.excerpt || '',
...
(function(w) {
  ...
  R.BlogCard = {
    ...
    render: function(data) {
      var excerpt = data.excerpt || (data.content ? data.content.substring(0, 120) : '');
      return CR.renderTemplate(this.template, {
        URL: CR.sanitizeUrl(data.url),
        IMAGE: data.image || '',
        TITLE: CR.escapeHtml(data.title),
        EXCERPT: CR.escapeHtml(excerpt),
        DATE: data.date || '',
        AUTHOR: CR.escapeHtml(data.author),
    ...
    renderAll: function(posts) {
      var self = this;
      return posts.map(function(p) { return self.render(p); }).join('');
    }
  };
})(window);
...
(function(w) {
  ...
  R.Navigation = {
    render: function(data) {
      return this.renderMenu(data.items || []);
    },
    ...
    renderMenu: function(items) {
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        if (hasChildren) {
          html += '<a href="' + this.sanitizeUrl(item.url) + '" class="nav-link dropdown-toggle" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">' + this.escapeHtml(item.title) + '</a>';
          html += '<div class="dropdown-menu">' + this.renderMenu(item.children) + '</div>';
        } else {
          html += '<a href="' + this.sanitizeUrl(item.url) + '" class="nav-link">' + this.escapeHtml(item.title) + '</a>';
        }
        html += '</li>';
      }
      html += '</ul>';
    ...
    renderMobile: function(data) {
      ...
      for (var i = 0; i < items.length; i++) {
        var item = items[i];
        html += '<a href="' + this.sanitizeUrl(item.url) + '" class="mobile-nav-link">' + this.escapeHtml(item.title) + '</a>';
        if (item.children && item.children.length) {
          html += '<div class="mobile-nav-sub">' + this.renderMobile({ items: item.children }) + '</div>';
        }
      }
      return html;
    },
