...
- **2026-07-31 Visual Customizer Phase B.7 — Placeholder polish for unimplemented tools** (commit `511b427`): `Property_Registry::TOOLS` — `spacing` + `assets` flipped `implemented => true` (both proven working; only animation/responsive/content remain placeholders). `data/component-parts.php` — hero gains `animation` part (animation-type/animation-delay/animation-duration → `hero_animation`, `hero_animation_delay`, `hero_animation_duration`) so the Animation tool appears live in the hero palette as a placeholder — proving future tools plug in via metadata, no code changes. Bridge (`admin/js/customizer-visual-editor.js`) — placeholder tools no longer `disabled`: clickable with dashed border + `SOON` badge (`text-transform: uppercase` — rendered text is `SOON`), click shows `.vc-tool-soon-notice` ("The Animation tool is coming soon.") with dismiss button instead of loading panels; All stays active; tools cached in module-scoped `toolsCache` (NOT `PhantomVC` — that global doesn't exist; first buggy version threw TypeError → "Failed to load the inspector." error panel, caught by E2E). CSS: `.vc-tool-soon` dashed + hover, `.vc-tool-soon-badge`, `.vc-tool-soon-notice` + dismiss. Tests: Property_Registry_Test updated flags, Component_Metadata_Test hero tools now `['colors','typography','spacing','assets','animation']`, Inspector_Tools_Test new `test_animation_tool_renders_animation_part`. E2E hardened: new `wait_inspector_has()` poller replaces fixed 1800ms waits in tool phases (fetch races were flaky — 4D failed once, 4C once); 4G polls crumb text directly (stale HTML contained "Primary Button" header text); 4H asserts `SOON` case-insensitively (CSS uppercases it). **E2E 21/21 PASS (4H new: Animation placeholder click → soon notice + All stays active + dismiss works), PHPUnit 592 tests/12521 assertions OK, smoke-packs ALL PASS, debug.log absent, 4 files md5-verified.** Phase B roadmap COMPLETE.
- **2026-07-31 Visual Customizer Phase B.5 — Selection breadcrumb + part capture** (commit `90c2348`): `selection-engine.js` — `findPartElement()`/`getPartData()` walk up from click target to component root for `data-part`; `PAGE_LABELS` map (body `*-page` class → label); `buildBreadcrumb()` → "Homepage › Hero › Primary Button" (`\u203A`); hover tooltip + overlay part-aware; `vc-element-selected` posts `part`/`partLabel`/`breadcrumb`. Pane: `loadInspector(..., done)` → `renderSelectionCrumb()` (`.vc-selection-crumb` bar prepended + `.vc-panel-header.vc-part-active` highlight on `data-panel` match). `frontend/html/index.html` hero tagged `data-part` + `data-part-label` (heading/description/button_primary/button_secondary ×3 slides, background div, image imgs). CSS: `.vc-selection-crumb` + `.vc-part-active` appended. **E2E 19/19 PASS** (2 new phases: heading click → crumb + heading panel highlighted; button click → button_primary panel; swiper note — scope clicks to `.swiper-slide-active`). PHPUnit 591/12509 OK, smoke-packs ALL PASS.
- **2026-07-31 Visual Customizer Phase B.0/B.1 — Property Registry + Universal Tool Engine** (commit `662da49`): Generic `Property_Registry` (`includes/Design/class-property-registry.php`) — 7 tools (colors/typography/spacing/assets/animation/responsive/content, only colors+typography implemented), ~30 generic properties keyed to tools with type/default/min/max/step/unit/responsive/options. `Component_Metadata` (`includes/Design/class-component-metadata.php` + `data/component-parts.php`) — 14 components self-describe: part→property mappings with label overrides; tools derived from property tools in registry order (never hard-coded). `Control_Renderer` (`includes/Inspector/class-control-renderer.php`) — single shared panel/field renderer (color/slider/range/select/toggle/textarea/text) for metadata AND legacy paths; `Inspector_Factory::render_panels()` now tool-filtered (`tool` param, unknown tool normalized to `''` via `tool_exists()`, metadata components via `get_metadata_panels()`, legacy `$component->tabs` fallback unchanged, assets panel only when tool `''`/`assets`). REST inspector GET accepts `tool` + returns `data.tools`. Bridge (`admin/js/customizer-visual-editor.js`) — tool palette (`renderToolPalette`) with All button + 7 tool buttons (unimplemented disabled `vc-tool-soon`), `selectTool()`, `currentTool` state, palette styles in `admin/css/customizer-visual-editor.css`. **591 tests/12509 assertions OK, php -l 0 errors, smoke-packs ALL PASS, Customizer E2E 17/17 PASS** (4 new tool phases: palette renders, Colors filters to color parts only, Typography filters to typography parts only, All restores overview). E2E Phase 5 hardened — color var check now polls up to 30s for preview reload (fixed 2.5s wait was flaky). Note: Phase 7 of E2E POSTs `#123456` to color_primary at run end, so every run starts from `#123456` — expected. Next: B.5 selection breadcrumb/part capture in `selection-engine.js`; B.7 placeholder polish.
...
- **2026-07-31 Customizer E2E — 4 bugs found & fixed** (commit `6db3afc`): (1) **CRITICAL** `templates/shell.php:249` `wp_kses_post($html)` stripped doctype/head/link/style/script/meta → frontend unstyled (styleCount=0, headLen=240) AND Customizer preview iframe head empty. Fixed: `echo $html;` (values already escaped at injection points). Frontend now: doctype html, 11 stylesheets, 15 scripts, `phantom-inline-css` present. (2) **CRITICAL** `admin/js/customizer-preview.js` auto-bind used 1-arg `wp.customize(id)` which returns `undefined` pre-creation (settings are created later via pane postMessage sync — `customize_preview_settings` inline prints AFTER footer scripts, and `customize-preview.min.js` creates values in jQuery-ready). 0/256 binds attached; blogname bind worked only because 2-arg `wp.customize(id, cb)` is the deferred `when()` form. Fixed: same 2-arg form → `--primary--color` updates live (verified 3 CSS vars change on pane edit). (3) `DesignStudioPage::body_class()` type-hinted `array` but `admin_body_class` filter passes a **string** → fatal on every admin page. Fixed: `body_class(string): string`. (4) `Array_Textarea_Control` content_template used `data.input_attrs.rows || 5` — core `to_json()` never exports `input_attrs` → template `TypeError`; guarded with `<# if ( data.input_attrs && data.input_attrs.rows ) #>`. Also wired `ast-array-textarea` into `Control_Base` map + Customizer routing with proper sanitize. **Production Customizer architecture (verified E2E)**: 10 panels (7 phantom: phantom_branding/global_colors/global_typography/presets/responsive/performance/integrations + 3 core), 26 sections (11 phantom), 289 controls, 314 settings, 16 ast-toggle, 34 ast-color, 3 ast-select, 25 control types. The 767-entry registry/46 sections + 16 legacy panels are the **Design Studio/REST design system**, dev-mode-gated (PHANTOM_DEV_MODE) — NOT shown in production Customizer. **E2E `customizer-e2e.py` (in `%TEMP%\opencode\`): all 8 phases PASS** — login, inventory, phantom panels/sections, preview iframe 470 CSS vars, live toggle/color binding, publish (save enables only when a value actually changes), REST GET /settings (50/page, short keys like `color_primary` — POST with `settings[color_primary]` returns 200), 0 console errors. Notes: `phantom_hero_enable_responsive`/`phantom_hero_fit` are dev-gated (E2E no-ops); settings without partial refresh trigger full preview iframe reload (E2E re-locates frame); debug.log absent; mu-plugin debug aid removed.
